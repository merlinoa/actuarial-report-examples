...
pander(eformat(smry, colnames = c("Paid[^1]", "Incurred[^2]", "Ultimate[^3]", "Case OS", "IBNR", "Total OS")))
```
...
>Notes:  
>
>>1: See Exhibit II, Sheet 1.  
>>2: See Exhibit III, Sheet 1.  
>>3: See Exhibit I.
...
pander(eformat(ult_selection, colnames = c("Paid Projection", "Incurred Projection", "Selected Ultimate")), 
       caption = "Selection of Estimated Ultimate Loss & ALAE")
```

\pagebreak

```{r paid_projection_exhibit, echo = FALSE}
pander(eformat(paid_proj, colnames = c("Paid", "LDF", "Ultimate")),
...
pander(eformat(incurred_proj, colnames = c("Incurred", "LDF", "Ultimate")))
```
